...
interface TaskCardDetailsInProgressProps {
  epic: Epic
  currentCost: number
...
export function TaskCardDetailsInProgress({
  epic,
  currentCost,
  className,
}: TaskCardDetailsInProgressProps) {
  const percentage = epic.budget ? Math.round((currentCost / epic.budget) * 100) : 0

  return (
    <div className={cn('flex-1 space-y-5', className)}>
