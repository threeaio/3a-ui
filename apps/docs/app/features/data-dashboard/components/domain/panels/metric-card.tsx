...
import { useProjectData, useTasksData } from '../../../data-context'
...
export const MetricCard: React.FC<MetricCardProps> = ({ name, value, change, trend, suffix = '', metricType }) => {
  ...
  const renderSubline = () => {
    switch (metricType) {
      ...
      case 'budget':
        return `$${project.currentSpend.toLocaleString('de-DE')}/$${project.budget.toLocaleString('de-DE')}`
      case 'timeRemaining': {
        ...
        return `End: ${formatDate(endDate)} (${totalDays} days total)`
      }
      default:
        return null
    }
